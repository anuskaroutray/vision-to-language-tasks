...
class Vocab(object):
    ...
    def __init__(self, counter, max_size=None, min_freq=1, specials=('<UNK>', '<PAD>'),
        ...
        if self.UNK in specials:  # hard-coded for now
            unk_index = specials.index(self.UNK)  # position in list
    ...
    def lookup_indices(self, tokens):
        indices = [self.__getitem__(token) if self.__getitem__(token) is not None else 0 for token in tokens]
        return indices
...
def build_vocab_from_iterator(
    ...
    word_vocab = Vocab(ordered_dict, min_freq=min_freq, specials=specials, specials_first=specials_first)
    return word_vocab